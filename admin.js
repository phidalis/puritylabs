...
const COLLS = {
  ...
  customers:{ col: 'customers',     key: c => c.id || c.email },
  coupons:  { col: 'coupons',       key: c => c.id || c.code },
  promo:    { col: 'promotions',    key: p => p.id },
  reviews:  { col: 'reviews',       key: r => r.id },
...
/* Initial (pre-live) catalog. Comes from catalog.js which is built from the
   real site data — transactional collections (orders, customers, coupons,
   promotions, reviews, visits, media) always start empty. Live Firestore data
   overlays these via the firestoreLoad() watchers, so the dashboard's product
   list is never wiped. */
...
function initDB() {
  ...
  DB = {
    ...
    cats: cat.cats,
    orders: [], customers: [], coupons: [], promo: [], reviews: [], visits: [], media: [],
    users: [],
    nav: cat.nav,
    content: cat.content,
...
function firestoreLoad() {
  ...
  const watch = (col, assign, sort) => {
    fsUnsubs.push(PDB.watchCol(col, list => {
      fsKeys[col] = keysOf(list);
      if (list && list.length) assign(list);
      refreshAdminPanel();
    }));
  };

  watch('products', v => { DB.products = sort ? withPos(v).sort(byPos) : withPos(v); }, true);
  watch('categories', v => { DB.cats = v; });
  watch('orders', v => { DB.orders = v; });
  watch('customers', v => { DB.customers = v; });
  watch('coupons', v => { DB.coupons = v; });
  watch('promotions', v => { DB.promo = v; });
  watch('reviews', v => { DB.reviews = v; });
...
function persistAll() {
  ...
  syncCollection('customers', DB.customers, COLLS.customers.key);
  syncCollection('coupons', DB.coupons, COLLS.coupons.key);
  syncCollection('promotions', DB.promo, COLLS.promo.key);
  syncCollection('reviews', DB.reviews, COLLS.reviews.key);
...
/* persistAll() syncs the in-memory admin state into Firestore: the product
   ...
   written; transactional collections (orders, customers, coupons,
   promotions, reviews, visits, media) are only written once real data exists,
   so no fake/demo data is ever pushed. */
function publishCatalog() {
  askConfirm('Publish catalog?', 'Writes the product catalog (products, categories, navigation, content and settings) into Firestore. Orders, customers, coupons, promotions, reviews and visits are never touched. Existing Firestore edits are kept. Continue?', () => {
    persistAll();
...
const LOCATIONS = {
  ...
  customers: 'Customers', users: 'Users & Roles', coupons: 'Coupons', promotions: 'Promotions', reviews: 'Reviews',
...
const NAV_ITEMS = [
  ...
  { id: 'promotions', icon: 'fa-bullhorn',      label: 'Promotions' },
...
function renderDashboard() {
  ...
  const stats = [
    ...
    { icon: 'fa-users', cls: 'red', val: DB.customers.length, lbl: 'Customers', sub: '+' + spend30 + ' orders (30d)', trend: 'up' }
  ];
  ...
  $('#panel-dashboard').innerHTML = `
    ...
    <div class="grid three" style="margin-top:18px;">
      ...
      <div class="card">
        <div class="card-head"><div><h3>Pending reviews</h3><p>Awaiting moderation</p></div>
        <button class="btn btn-ghost btn-sm" data-goto="reviews">Review</button></div>
        ${DB.reviews.filter(r => r.status === 'pending').slice(0,4).map(r => `
          <div class="recent-item">
            <div class="stars">${starsHTML(r.rating)}</div>
            <div class="recent-meta"><b>${esc(r.author)}</b><span>on ${esc(r.product)}</span></div>
            <span class="badge warn">pending</span>
          </div>`).join('') || '<div class="empty"><p>No pending reviews.</p></div>'}
      </div>
    </div>`;
}
...
function starsHTML(n) {
  return Array.from({ length: 5 }, (_, i) => '<i class="fa-solid ' + (i < n ? 'fa-star' : 'fa-star-half-stroke') + '"></i>').join('');
}
...
function viewProductPage(id) {
  ...
  openModal('Product page', `
    ...
    <div class="form-field"><label>Live page URL</label>
      <input readonly value="pages/products/${esc(p.slug)}.html">
    </div>`,
    '<button class="btn btn-ghost" data-close-modal>Close</button><a class="btn btn-primary" target="_blank" href="pages/products/' + esc(p.slug) + '.html"><i class="fa-solid fa-up-right-from-square"></i> Open page</a>');
}
...
function renderCustomers() {
  $('#panel-customers').innerHTML = `
    <div class="toolbar">
      <h3 style="color:var(--ink);font-size:1rem;">${DB.customers.length} customers</h3>
      <div class="spacer"></div>
      <input type="search" placeholder="Search..." id="custSearch" style="max-width:220px;">
      <button class="btn btn-primary" data-cust-form><i class="fa-solid fa-plus"></i> Add Customer</button>
    </div>
    <div class="table-wrap">
      <table class="data">
        <thead><tr><th>Customer</th><th>Orders</th><th>Total spent</th><th>Joined</th><th>Last order</th><th style="text-align:right;">Actions</th></tr></thead>
        <tbody>
          ${DB.customers.map(c => {
            const last = DB.orders.filter(o => o.email === c.email).sort((a, b) => new Date(b.date) - new Date(a.date))[0];
            return `<tr>
              <td><div class="td-prod"><div class="avatar-sm">${esc(initials(c.name))}</div><div><b style="color:var(--ink);">${esc(c.name)}</b><small>${esc(c.email)}</small></div></div></td>
              <td><span class="badge ok">${c.orders}</span></td>
              <td><b style="color:var(--ink);">${money(c.totalSpent)}</b></td>
              <td style="white-space:nowrap;">${fmtDate(c.joined)}</td>
              <td style="white-space:nowrap;">${last ? fmtDate(last.date) : '—'}</td>
              <td><div class="actions"><button class="icon-action" data-cust-edit="${c.id}"><i class="fa-solid fa-pen"></i></button>
              <button class="icon-action danger" data-cust-del="${c.id}"><i class="fa-solid fa-trash"></i></button></div></td>
            </tr>`;
          }).join('')}
        </tbody>
      </table>
  ...
  $('#custSearch').addEventListener('input', (e) => {
    ...
    $$('#panel-customers tbody tr').forEach(tr => {
      tr.style.display = tr.textContent.toLowerCase().includes(q) ? '' : 'none';
    });
  });
  $$('[data-cust-form]').forEach(b => b.addEventListener('click', () => customerForm()));
  $$('[data-cust-edit]').forEach(b => b.addEventListener('click', () => customerForm(b.dataset.custEdit)));
  $$('[data-cust-del]').forEach(b => b.addEventListener('click', () => {
    const c = byId('customers', b.dataset.custDel);
    askConfirm('Remove customer?', c.name + ' (' + c.email + ') will be removed.', () => {
      DB.customers = DB.customers.filter(x => x.id !== c.id); persistAll(); renderCustomers(); toast('Customer removed.');
    });
  }));
}
...
function renderUsers() {
  ...
  $('#panel-users').innerHTML = `
    <div class="toolbar">
      ...
      <div class="spacer"></div>
      <p style="font-size:.82rem;color:var(--muted);">Sign-ups on the storefront create <span class="badge muted">customer</span> roles; change a role here and that account gets admin (or loses it) immediately.</p>
    </div>
    <div class="table-wrap">
      <table class="data">
        ...
        <tbody>
          ${DB.users.map(u => {
            ...
            return `<tr>
              ...
              <td><div class="actions">
                ${u.role === 'admin'
                  ? `<button class="icon-action" data-user-role="customer" data-user-id="${u.id}" title="Demote to customer"><i class="fa-solid fa-user-minus"></i></button>`
    ...
    <p style="font-size:.78rem;color:var(--muted);margin-top:12px;">Roles come from the <code>users/{uid}.role</code> document. Admins are the only ones allowed to change roles. You cannot demote your own account.</p>`;
  $$('[data-user-role]').forEach(b => {
    b.addEventListener('click', () => setUserRole(b.dataset.userId, b.dataset.userRole));
  });
}
...
function setUserRole(uid, role) {
  ...
  askConfirm('Change role?', (u.name || u.email) + ' will be ' + verb + '.', () => {
    PDB.setDoc('users', uid, { role: role }, { merge: true })
      .then(() => toast('Role updated to ' + role + ' (takes effect next page load).', 'ok'))
...
function customerForm(id) {
  const isNew = !id;
  const c = isNew ? { name: '', email: '', orders: 0, totalSpent: 0, joined: todayStr() } : byId('customers', id);
  openModal(isNew ? 'Add Customer' : 'Edit Customer', `
    <div class="form-grid">
      <div class="form-field"><label>Name</label><input id="cu-name" value="${esc(c.name)}"></div>
      <div class="form-field"><label>Email</label><input id="cu-email" value="${esc(c.email)}"></div>
      <div class="form-field"><label>Orders</label><input id="cu-orders" type="number" value="${c.orders}"></div>
      <div class="form-field"><label>Total spent ($)</label><input id="cu-total" type="number" step="0.01" value="${c.totalSpent}"></div>
    </div>`,
    '<button class="btn btn-ghost" data-close-modal>Cancel</button><button class="btn btn-primary" data-save-cust>Save</button>');
  $('#modalFoot [data-save-cust]').addEventListener('click', () => {
    const name = $('#cu-name').value.trim();
    if (!name) { toast('Name required.', 'error'); return; }
    if (isNew) DB.customers.push({ id: uid(), name, email: $('#cu-email').value.trim(), orders: +$('#cu-orders').value || 0, totalSpent: +$('#cu-total').value || 0, joined: todayStr() });
    else Object.assign(c, { name, email: $('#cu-email').value.trim(), orders: +$('#cu-orders').value || 0, totalSpent: +$('#cu-total').value || 0 });
    persistAll(); closeModal(); renderCustomers(); toast('Customer saved.');
  });
}
...
function renderPromotions() {
  $('#panel-promotions').innerHTML = `
    <div class="toolbar">
      <h3 style="color:var(--ink);font-size:1rem;">Active campaigns</h3>
      <div class="spacer"></div>
      <button class="btn btn-primary" data-new-promo><i class="fa-solid fa-plus"></i> New Promotion</button>
    </div>
    <div class="grid three">
      ${DB.promo.map(pr => `
        <div class="card" style="border-top:4px solid ${esc(pr.bg || '#2E7D32')};">
          <div class="card-head"><div><h3>${esc(pr.title)}</h3><p>${esc(pr.label)}</p></div>
            <span class="badge ${pr.active ? 'ok' : 'muted'}">${pr.active ? 'live' : 'off'}</span></div>
          <div class="table-wrap" style="margin-bottom:14px;">
            <table class="data" style="min-width:0;">
              <tbody>
                <tr><td style="color:var(--muted);">Type</td><td><b>${fmtPromoType(pr.type)}</b></td></tr>
                <tr><td style="color:var(--muted);">Value</td><td><b>${pr.type === 'percent_off' ? pr.value + '%' : pr.type === 'free_shipping' ? 'Free shipping' : 'Buy more / ' + pr.value + '%'}</b></td></tr>
                <tr><td style="color:var(--muted);">Ends</td><td style="white-space:nowrap;">${fmtDate(pr.ends)}</td></tr>
              </tbody>
            </table>
          </div>
          <div class="actions">
            <button class="btn btn-ghost btn-sm" data-edit-promo="${pr.id}"><i class="fa-solid fa-pen"></i> Edit</button>
            <button class="btn btn-ghost btn-sm" data-toggle-promo="${pr.id}">${pr.active ? 'Deactivate' : 'Activate'}</button>
            <button class="icon-action danger" data-del-promo="${pr.id}"><i class="fa-solid fa-trash"></i></button>
          </div>
        </div>`).join('')}
    </div>
    <div class="card" style="margin-top:18px;">
      ...
      <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
        <input id="ann-label" value="${esc(DB.content.announcementLabel)}" style="max-width:110px;" placeholder="Label (NEW)">
        <input id="ann-text" value="${esc(DB.content.announcement)}" style="flex:1;min-width:200px;" placeholder="Announcement text">
        <button class="btn btn-primary" id="ann-save">Update announcement</button>
      </div>
    </div>`;
  $$('[data-new-promo]').forEach(b => b.addEventListener('click', () => promoForm()));
  $$('[data-edit-promo]').forEach(b => b.addEventListener('click', () => promoForm(b.dataset.editPromo)));
  $$('[data-toggle-promo]').forEach(b => b.addEventListener('click', () => {
    const pr = byId('promo', b.dataset.togglePromo); pr.active = !pr.active; persistAll(); renderPromotions();
    toast(pr.title + (pr.active ? ' is live.' : ' deactivated.'));
  }));
  $$('[data-del-promo]').forEach(b => b.addEventListener('click', () => {
    const pr = byId('promo', b.dataset.delPromo);
    askConfirm('Delete promotion?', 'Delete "' + pr.title + '"?', () => {
      DB.promo = DB.promo.filter(x => x.id !== pr.id); persistAll(); renderPromotions(); toast('Promotion deleted.');
    });
  }));
  $('#ann-save').addEventListener('click', () => {
    DB.content.announcement = $('#ann-text').value;
    DB.content.announcementLabel = $('#ann-label').value;
    persistAll(); toast('Announcement updated.');
  });
}

function fmtPromoType(t) {
  return { percent_off: 'Percent off', free_shipping: 'Free shipping', buy_more: 'Volume discount' }[t] || t;
}

function promoForm(id) {
  const isNew = !id;
  const pr = isNew ? { title: '', type: 'percent_off', value: 10, label: '', bg: '#2E7D32', active: true, ends: daysAgo(-7) } : byId('promo', id);
  openModal(isNew ? 'New Promotion' : 'Edit Promotion', `
    <div class="form-grid">
      <div class="form-field full"><label>Campaign title</label><input id="pm-title" value="${esc(pr.title)}"></div>
      <div class="form-field"><label>Type</label><select id="pm-type">
        <option value="percent_off" ${pr.type === 'percent_off' ? 'selected' : ''}>Percent off</option>
        <option value="free_shipping" ${pr.type === 'free_shipping' ? 'selected' : ''}>Free shipping</option>
        <option value="buy_more" ${pr.type === 'buy_more' ? 'selected' : ''}>Buy more, save more</option>
      </select></div>
      <div class="form-field"><label>Value</label><input id="pm-value" type="number" value="${pr.value}"></div>
      <div class="form-field"><label>Badge label</label><input id="pm-label" value="${esc(pr.label)}"></div>
      <div class="form-field"><label>Ends</label><input id="pm-exp" type="date" value="${new Date(pr.ends).toISOString().slice(0, 10)}"></div>
      <div class="form-field full"><label>Banner color</label><select id="pm-bg">
        ${['#2E7D32', '#10241a', '#1b4130', '#0d1f16', '#C07E1F'].map(c => `<option value="${c}" ${pr.bg === c ? 'selected' : ''}>${c}</option>`).join('')}
      </select></div>
    </div>`,
    '<button class="btn btn-ghost" data-close-modal>Cancel</button><button class="btn btn-primary" data-save-promo>Save</button>');
  $('#modalFoot [data-save-promo]').addEventListener('click', () => {
    const title = $('#pm-title').value.trim();
    if (!title) { toast('Title required.', 'error'); return; }
    const rec = { title, type: $('#pm-type').value, value: +$('#pm-value').value || 0, label: $('#pm-label').value.trim(), bg: $('#pm-bg').value, active: pr.active !== undefined ? pr.active : true, ends: new Date($('#pm-exp').value + 'T23:59:59').toISOString() };
    if (isNew) DB.promo.push(Object.assign({ id: uid() }, rec));
    else Object.assign(pr, rec);
    persistAll(); closeModal(); renderPromotions(); toast('Promotion saved.');
  });
}
...
function renderReviews() {
  ...
  const nPending = all.filter(r => r.status === 'pending').length;
  $('#panel-reviews').innerHTML = `
    <div class="toolbar">
      <h3 style="color:var(--ink);font-size:1rem;">${nPending} of ${all.length} pending moderation</h3>
      <div class="spacer"></div>
      <button class="btn btn-primary" data-new-review><i class="fa-solid fa-plus"></i> Add Review</button>
    </div>
    <div class="grid two">
      ${all.map(r => `
        <div class="card">
          <div class="review-head" style="margin-bottom:8px;">
            <div class="avatar-sm">${esc(initials(r.author))}</div>
            <div style="flex:1;">
              <b style="color:var(--ink);font-size:.92rem;">${esc(r.author)}</b>
              <div class="stars" style="font-size:.78rem;">${starsHTML(r.rating)}</div>
            </div>
            <span class="badge ${r.status === 'approved' ? 'ok' : r.status === 'pending' ? 'warn' : 'red'}">${esc(r.status)}</span>
          </div>
          <p style="font-size:.88rem;color:var(--text);margin-bottom:6px;">on <b style="color:var(--ink);">${esc(r.product)}</b></p>
          <p style="font-size:.9rem;color:var(--text);margin-bottom:14px;">"${esc(r.text)}"</p>
          <div style="font-size:.76rem;color:var(--muted);margin-bottom:12px;">${fmtDate(r.date)}</div>
          <div class="actions">
            ${r.status !== 'approved' ? `<button class="btn btn-ghost btn-sm" data-approve-review="${r.id}"><i class="fa-solid fa-check"></i> Approve</button>` : `<button class="btn btn-ghost btn-sm" data-unapprove-review="${r.id}"><i class="fa-solid fa-rotate-left"></i> Unapprove</button>`}
            <button class="btn btn-ghost btn-sm" data-edit-review="${r.id}"><i class="fa-solid fa-pen"></i> Edit</button>
            <button class="icon-action danger" data-del-review="${r.id}"><i class="fa-solid fa-trash"></i></button>
          </div>
        </div>`).join('') || '<div class="empty"><i class="fa-solid fa-star"></i><p>No reviews yet.</p></div>'}
    </div>`;
  $$('[data-approve-review]').forEach(b => b.addEventListener('click', () => {
    const r = byId('reviews', b.dataset.approveReview); r.status = 'approved'; persistAll(); renderReviews(); renderNav(); toast('Review approved.');
  }));
  $$('[data-unapprove-review]').forEach(b => b.addEventListener('click', () => {
    const r = byId('reviews', b.dataset.unapproveReview); r.status = 'pending'; persistAll(); renderReviews(); renderNav(); toast('Review marked pending.');
  }));
  $$('[data-edit-review]').forEach(b => b.addEventListener('click', () => reviewForm(b.dataset.editReview)));
  $$('[data-new-review]').forEach(b => b.addEventListener('click', () => reviewForm()));
  $$('[data-del-review]').forEach(b => b.addEventListener('click', () => {
    const r = byId('reviews', b.dataset.delReview);
    askConfirm('Delete review?', 'Remove review by ' + r.author + '?', () => {
      DB.reviews = DB.reviews.filter(x => x.id !== r.id); persistAll(); renderReviews(); renderNav(); toast('Review deleted.');
    });
  }));
}
...
function reviewForm(id) {
  let isNew = !id;
  let r = isNew ? { product: DB.products[0].name, author: '', rating: 5, text: '', status: 'pending', date: todayStr() } : byId('reviews', id);
  if (r == null) { r = { product: DB.products[0].name, author: '', rating: 5, text: '', status: 'pending', date: todayStr() }; isNew = true; }
  openModal(isNew ? 'Add Review' : 'Edit Review', `
    <div class="form-grid">
      <div class="form-field"><label>Product</label><select id="rv-prod">
        ${DB.products.map(p => `<option value="${esc(p.name)}" ${r.product === p.name ? 'selected' : ''}>${esc(p.name)}</option>`).join('')}
      </select></div>
      <div class="form-field"><label>Author</label><input id="rv-author" value="${esc(r.author)}"></div>
      <div class="form-field"><label>Rating (1-5)</label><input id="rv-rating" type="number" min="1" max="5" value="${r.rating}"></div>
      <div class="form-field"><label>Status</label><select id="rv-status">
        <option value="pending" ${r.status === 'pending' ? 'selected' : ''}>Pending</option>
        <option value="approved" ${r.status === 'approved' ? 'selected' : ''}>Approved</option>
        <option value="rejected" ${r.status === 'rejected' ? 'selected' : ''}>Rejected</option>
      </select></div>
      <div class="form-field full"><label>Review text</label><textarea id="rv-text">${esc(r.text)}</textarea></div>
    </div>`,
    '<button class="btn btn-ghost" data-close-modal>Cancel</button><button class="btn btn-primary" data-save-review>Save</button>');
  $('#modalFoot [data-save-review]').addEventListener('click', () => {
    const author = $('#rv-author').value.trim();
    if (!author) { toast('Author required.', 'error'); return; }
    const rec = { product: $('#rv-prod').value, author, rating: Math.min(5, Math.max(1, +$('#rv-rating').value || 5)), text: $('#rv-text').value.trim(), status: $('#rv-status').value, date: r.date || todayStr() };
    if (isNew) DB.reviews.push(Object.assign({ id: uid() }, rec));
    else Object.assign(r, rec);
    persistAll(); closeModal(); renderReviews(); renderNav(); toast('Review saved.');
  });
}
      ...
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="table-wrap">
          <table class="data">
            ...
            <tbody>
              ${[['Home','index.html','published'],['Cart','cart.html','published'],['Checkout','checkout.html','published'],['Nasal Sprays','pages/nasal-sprays.html','published'],['Sitemap','sitemap.xml','published']].map(([n,f,s]) => `
...
function settingsHTML(tab, s) {
    ...
    return `
      <div class="card" style="max-width:760px;">
        ...
        <p style="font-size:.9rem;color:var(--muted);margin-bottom:18px;line-height:1.6;">
          ...
          There is no offline seed file — <code>data/data.json</code> was removed; the built-in
          catalog (<code>catalog.js</code>) only boots the panel before live data loads.
          Transactions, customers, coupons and reviews are Firestore-only.
        </p>
        <div class="card" style="border:1px dashed var(--line);">
          <h4 style="margin-bottom:8px;">Publish catalog</h4>
          <p style="font-size:.85rem;color:var(--muted);margin-bottom:12px;">Writes the product catalog (products, categories, navigation, content, settings) to Firestore. Orders, customers, coupons, promotions, reviews and visits are never touched.</p>
  ...
  return `
    ...
    <div class="card" style="max-width:720px;margin-top:18px;">
      ...
      <p style="font-size:.86rem;color:var(--muted);margin-bottom:14px;">Deletes all transactional data (orders, customers, coupons, promotions, reviews, visits and media) from Firestore. Products, categories, navigation, content and settings are <b>never</b> removed.</p>
...
function bindSettings(tab, s) {
  if (tab === 'general') {
    ...
    $('#settingsView [data-reset-all]')?.addEventListener('click', () => {
      askConfirm('Clear transactional data?', 'Orders, customers, coupons, promotions, reviews, visits and media are deleted from Firestore. Products, categories, navigation, content and settings are kept untouched.', () => {
        wipeCols(['orders', 'customers', 'coupons', 'promotions', 'reviews', 'visits', 'media']);
...
function initNotifs() {
  const n = DB.reviews.filter(r => r.status === 'pending').length + DB.orders.filter(o => o.status === 'pending').length;
  const dot = $('#notifDot');
  ...
  $('#notifBtn').addEventListener('click', () => {
    const items = [
      ...
      ...DB.reviews.filter(r => r.status === 'pending').map(r => ({ icon: 'fa-star', text: 'Review pending: ' + r.author + ' on ' + r.product, act: 'reviews' }))
    ];
