...
function ageGate() {
  const gate = $('#ageGate');
  if (!gate) return;
  if (localStorage.getItem('ageVerified') || sessionStorage.getItem('ageVerified')) { gate.classList.add('hidden'); return; }
  $('#ageNo').addEventListener('click', (e) => { e.preventDefault(); window.location.href = 'index.html'; });
  $('#ageYes').addEventListener('click', (e) => {
    e.preventDefault();
    const k = $('#ageRemember') && $('#ageRemember').checked ? localStorage : sessionStorage;
    k.setItem('ageVerified', '1');
